...
from movie.models import UserFilms, UserWatchList
...
def get_watchlist_order(user) -> int:
    existing_films = UserWatchList.objects.filter(user=user)
    if not existing_films.exists():
        return 1
    else:
        current_max = existing_films.aggregate(max_order=Max('order'))['max_order']
